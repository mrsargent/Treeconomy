...
export default async function handler(
  ...
  if (req.method === "POST") {
    ...
    const applied = applyParamsToScript(applyDoubleCborEncoding(compiledNft!), [pkh1, 1n]);
    ...
    //policy for the burn of the seed
    const appliedBurn = applyParamsToScript(applyDoubleCborEncoding(compiledNft!), [pkh1, 0n]);
    ...
    if (goodUtxo !== undefined) {
      ...
      metadataMap.set(fromText("name"), Data.to(fromText("Sapling NFT " + treeData.number)));
