...
export function parseAssetId(assetId: string) {
  ...
  export function claimTokens(
        ...
        numberOfTimesClaimed: bigint
    ) : WithdrawalInfo{
        
        if (numberOfTimesClaimed >= totalInstallments){
            return {
                canWithdraw: false,
                updatedTimesClaimed: totalInstallments
            }
        }

        const totalTime = endTime-startTime;
        const timeElapsed = currentTime - startTime;
        const intervalDuration = totalTime/totalInstallments;
        const currentInterval = timeElapsed/intervalDuration;
    
        if (currentInterval > numberOfTimesClaimed){
            return {
                canWithdraw: true,
                updatedTimesClaimed: currentInterval
            }
        }

        return {
            canWithdraw: false,
            updatedTimesClaimed: numberOfTimesClaimed
        }
    }
